...
thisScript <- "model_diagnostic_1cmt.qmd"

etas <- c("ETA-V1C", "ETA-CLC", "ETA-V1T", "ETA-CLT", "ETA-CORR")   
etas_est  <- c("ETA1//ETA-V1C", "ETA2//ETA-CLC", "ETA3//ETA-V1T-CLT")
...
drugs <- c("Ceftolozane" = "bisque4", "Tazobactam" ="deepskyblue4")
...
legend <- get_legend(gof[[1]] + theme(legend.position = "right") + 
 ...
 pm_grid(gof[[1]]/gof[[2]]) + plot_layout(ncol = 1, heights = c(10, 0.01)) + legend + plot_annotation(title = "One-compartment model") & theme(legend.position = "none")
...
eta_cor <- par  %>% 
  ...
  rename("ETA-V1C" = ETA1, "ETA-CLC" = ETA2, "ETA-V1T-CLT" = ETA3, ) %>% 
  distinct() %>% 
  select(-ID) 
