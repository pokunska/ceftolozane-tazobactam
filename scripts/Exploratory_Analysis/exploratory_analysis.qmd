...
pk_stud <- xdata_cmt %>% 
  ...
  st_aspng(stem = "parent_by_detail", dir = here(table_dir, "Obs_table"), textwidth = getOption("pmtables.text.width", 1)) 
...
tab_cat <- 
  ...
  stable() %>% 
  st_aspng(stem = "categorical_cov", dir = here(table_dir, "Cat_table"), textwidth = getOption("pmtables.text.width", 1)
           )
...
tab_cont <- timeIndCoDF %>%
  pt_cont_long(
    cols = c("HEIGHT", "BW", "AGE", "SOFA", "PCT", "ELWI", "CI", "CRE", "ALB", "PROTEIN"), 
    table = covlab) %>% 
  st_new() %>% 
  st_files(r = NULL
  ...
  st_notes_str() %>%
  stable() %>% 
  st_aspng(stem = "continuous_cov", dir = here(table_dir, "Con_table"))
