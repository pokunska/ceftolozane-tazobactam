...
```{r htmltable parameters table, out.width='100%'}
st_as_image(fixed)
```
:::
...
::: {.content-visible when-format="html"}
```{r htmltable rests table, out.width='100%'}
st_as_image(rests)
```
:::
...
::: {.content-visible when-format="html"}
```{r htmltable add 95CI, out.width='100%'}
st_as_image(fixed)
```
:::
...
::: {.content-visible when-format="html"}
```{r htmltable add 95CI rests, out.width='100%'}
st_as_image(rests)
```
:::
...
```{r vpc legend}
# Calculate number of posterior samples and chains
nPost <- mod5_res$metadata()$iter_sampling / mod5_res$metadata()$thin
n_chains <- 4
...
# Enhance plot with points, scales, labels, theme, and legend
vpcPlot <- vpcPlotraw +
  ...
  theme(
    ...
    legend.text         = element_text(size = 9),          # ← mniejsza czcionka w legendzie
    legend.key.height   = unit(0.9, "lines"),              # opcjonalnie – nieco ciaśniejsze odstępy
    legend.margin       = margin(2, 0, 0, 0)
  ) +
  # Dummy geoms for legend – pełne nazwy
...
::: {.content-visible when-format="html"}
```{r htmltable bs1cmt params, out.width='100%'}
...
::: {.content-visible when-format="html"}
```{r htmltable bs2cmt params, out.width='100%'}
st_as_image(png_table)
```
:::
